...
import com.sist.jwt_mem.result.ResultData;
...
public class ApiMemberController {
    ...
    @PostMapping("/login")
    @ResponseBody
    public ResultData<Member> login(Member mvo, HttpServletResponse response) {
        ...
        if(mvo.getMId() != null){
            ...
            member = m_service.memInfo(mvo.getMId(), mvo.getMPw()); // 로그인 정보가 불일치하다면 반환값인 member은 null이 된다.
        ...
        // return m_map;
        return ResultData.of(cnt, msg, member);
    }
    ...
    @PostMapping("/logout")
    public ResultData<Member> logout(HttpServletResponse response) {
        // 쿠키에서 accessToken과 refreshToken을 삭제하여
        // 클라이언트에게 보낸다.
